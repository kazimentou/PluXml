...
class plxUpdater {
	...
	public function startUpdate($version='') {
		...
		/*
		$offset = array_search($version, array_keys($this->updateVersions));
		if($offset!='') {
			$this->updateVersions = array_slice($this->updateVersions, $offset+1, null, true);
		}
		* */
		$versions = null;
		if(!empty($version)) {
			$versions = array_filter($this->allVersions, function($value) use($version) {
				return version_compare($value, $version, '>');
			});
		} else {
	...
	public function getVersions() {
		...
		# Récupère l'ancien n° de version de Pluxml si possible
...
?>
			<ul>
<?php
		foreach($this->updateVersions as $num_version) {
?>
				<li>
...
<?php
			...
			$upd_filename = 'update_' . $num_version . '.php';
			...
			if(!class_exists($class_name)) {
				# Rétro-compabilité
				$class_name = preg_replace('#_0$#', '', $class_name);
			}
