...
class plxMedias
{
    ...
    public function __construct($path, $dir)
    {
        ...
        if (substr_count($maxUpload, 'K')) {
            $this->maxUpload['value'] = str_replace('K', '', $maxUpload) * 1024;
        } elseif (substr_count($maxUpload, 'M')) {
            $this->maxUpload['value'] = str_replace('M', '', $maxUpload) * 1024 * 1024;
        } elseif (substr_count($maxUpload, 'G')) {
            $this->maxUpload['value'] = str_replace('G', '', $maxUpload) * 1024 * 1024 * 1024;
        } else {
        ...
        if (substr_count($maxPost, 'K')) {
            $this->maxPost['value'] = str_replace('K', '', $maxPost) * 1024;
        } elseif (substr_count($maxPost, 'M')) {
            $this->maxPost['value'] = str_replace('M', '', $maxPost) * 1024 * 1024;
        } elseif (substr_count($maxPost, 'G')) {
            $this->maxPost['value'] = str_replace('G', '', $maxPost) * 1024 * 1024 * 1024;
        } else {
    ...
    private function _getDirFiles($dir)
    {
        ...
        foreach (array_filter(
            glob($src.'*'),
            function ($item) {
                return !preg_match('@\.tb\.\w+$@', $item);
            } # On rejette les miniatures
        ) as $filename) {
            if (is_dir($filename)) {
                continue;
            }
