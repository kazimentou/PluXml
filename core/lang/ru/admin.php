...
$LANG = array(
		...
		'L_CONFIG_ADVANCED_EMAIL_SENDING_TITLE_HELP' => 'Нужна помощь : <a href="https://wiki.pluxml.org/docs/customize/advancedconfig.html?highlight=smtp#envoi-d-e-mails">Документация PluXml</a>&nbsp;(fr).',
		'L_CONFIG_ADVANCED_EMAIL_METHOD_HELP' => 'Для sendmail не требуется настройка.',
		'L_CONFIG_ADVANCED_SMTP_SERVER_HELP' => 'Имя SMTP-сервера (пример : ssl0.ovh.net)',
		'L_CONFIG_ADVANCED_SMTP_USERNAME_HELP' => 'Имя пользователя на узле SMTP (пример : pluxml@monserveursmtp.com)',
		'L_CONFIG_ADVANCED_SMTP_PASSWORD_HELP' => 'Пароль пользователя на хосте SMTP',
		'L_CONFIG_ADVANCED_SMTP_PORT_HELP' => 'Номер порта хоста SMTP (по умолчанию : 465)',
		'L_CONFIG_ADVANCED_SMTPOAUTH_TITLE_HELP' => 'PluXml позволяет генерировать токены только для сервиса <a href="https://cloud.google.com">GMAIL (Google)</a>.<br>Нужна помощь в создании имени пользователя и секретного ключа : <a href="https://wiki.pluxml.org/docs/customize/advancedconfig.html?highlight=smtp#envoi-d-e-mails">Документация PluXml</a>&nbsp;(fr).',
		...
		'L_CONFIG_ADVANCED_SMTPOAUTH_TOKEN_HELP' => 'Сохраните идентификатор клиента и секретный код клиента, чтобы вы могли сгенерировать токен.',
		...
		'L_MAIL_TEST' => 'Отправить E-mail для теста',
		'L_MAIL_TEST_SUBJECT' => 'Проверка почты, отправленной на %s',
		'L_MAIL_TEST_SENT_TO' => 'Е-мейл отправлен %s. Проверьте свой почтовый ящик',
		'L_MAIL_TEST_FAILURE' => 'Вопрос для отправки электронной почты',
