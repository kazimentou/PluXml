...
if (!empty($_POST)) { # Création, mise à jour, suppression ou aperçu
    ...
    if (trim($_POST['title'])=='') {
        $_POST['title'] = L_DEFAULT_NEW_ARTICLE_TITLE;
    }
    ...
    if (sizeof($_POST['catId'])==1 and $_POST['catId'][0]=='draft') {
        $_POST['catId'][]='000';
    } else {
    ...
    if ((isset($_POST['update']) or isset($_POST['publish']) or isset($_POST['moderate'])) and isset($_POST['catId'])) {
        $_POST['catId'] = array_filter($_POST['catId'], function ($a) {
            return $a!="draft";
        });
    }
	# Si profil PROFIL_WRITER on vérifie l'id du rédacteur connecté et celui de l'article
    if ($_SESSION['profil']==PROFIL_WRITER and isset($_POST['author']) and $_SESSION['user']!=$_POST['author']) {
    ...
    if (!empty($_POST['preview'])) {
        ...
        if (empty($art['url'])) {
            $art['url'] = L_DEFAULT_NEW_ARTICLE_URL;
        }
		...
		<div class="col sml-12 med-7 lrg-8">
			...
			<div class="grid gridthumb">
				<div class="col sml-12">
                    ...
                    if ($src) {
                        echo "<img src=\"$src\" title=\"$thumbnail\" />\n";
                    }
					?>
					</div>
				</div>
