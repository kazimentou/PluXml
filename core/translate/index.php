...
Rechercher toutes les clés utilisées : cd core/translation/; php -f tokens.php. Afficher tokens.json</code></pre>
	</aside>
...
<?php
	...
	if(file_exists($filename)) {
		$usefulTokens = json_decode(file_get_contents($filename), true);
	}
	foreach($translations as $key=>$values) {
		...
		if($isComment) {
			$className = 'comment';
		} elseif(!empty($usefulTokens) and !in_array($key, $usefulTokens[$_SESSION['fichier']])) {
			$className = 'unused';
			$caption = 'Jamais utilisé';
			foreach($usefulTokens as $k=>$v) {
				if($k != $_SESSION['fichier'] and in_array($key, $usefulTokens[$k])) {
					$caption = 'A déplacer dans ' . $k;
					break;
				}
			}
			$tooltip = ' <span class="tooltip">' . $caption . '</span>';
		} else {
			$className = '';
		}
?>
...
?>
						<th><input type="hidden" name="token[<?= $i ?>]" value="<?= $prefix . $key ?>" /><span><?= $key ?></span><?= $tooltip ?></th>
<?php
			foreach(array_keys($langs) as $lang) {
...
<?php
	...
	$missingTokens = array_diff($usefulTokens[$_SESSION['fichier']], array_keys($translations));
	if(!empty($missingTokens)) {
		$alternativeFiles =array();
		foreach(FILES as $f) {
			if($f != $_SESSION['fichier']) {
				$filename = PLX_LANGS . $_SESSION['principale'] . '/' . $f . '.php';
				$content = file_get_contents($filename);
				if(preg_match_all('#\bL_\w+\b#', $content, $matches)) {
					sort($matches[0]);
					$alternativeFiles[$f] = array_unique($matches[0]);
				}
			}
		}
		foreach($missingTokens as $key) {
			$i++;
			$tooltip = '';
			foreach($alternativeFiles as $k=>$v) {
				if(in_array($key, $v)) {
					$tooltip = ' <span class="tooltip">Présent dans ' . $k . '</span>';
					break;
				}
			}
?>
...
<?php
			foreach(array_keys($langs) as $lang) {
?>
						<td><input name="<?= $lang ?>[<?= $i ?>]" value="" data-extra /></td>
<?php
			}
?>
						<td></td>
